...
BOTPRESS_TOKEN = st.secrets["botpress"]["api_key"]  # Add this in your Streamlit secrets
BOTPRESS_URL = 'https://messaging.botpress.cloud/v1/messages'
API_KEY = st.secrets["alpha_vantage"]["api_key"]

# --- STREAMLIT SETUP ---
st.set_page_config(page_title="💸 Budgeting + Botpress API Agent", layout="wide")
st.title("💸 Budgeting + Investment Planner + Botpress Agent (API Connected)")
...
def send_budget_to_botpress(conversation_id, summary):
    headers = {
    ...
    resp = requests.post(BOTPRESS_URL, headers=headers, json=payload)
    return resp.json()

# Generate budget summary
budget_summary = f"""
Income: ${income}, After tax: ${after_tax_income}, Expenses: ${total_exp}, 
Investments: ${total_inv}, Net Flow: ${net_flow}/mo, Projected Net Worth: ${df['NetWorth'].iloc[-1]}
"""

if st.button("Send Budget to Botpress"):
    st.info("Sending budget summary to Botpress...")
    convo_id = str(uuid.uuid4())  # New convo ID each time or persist as needed
    bot_response = send_budget_to_botpress(convo_id, budget_summary)
    st.subheader("🤖 Botpress Reply")
